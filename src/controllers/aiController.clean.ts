  ...
  try {
    ...
    const { response: aiData, modelUsed, attemptCount } = await callOptimalAI(
      [
        {
          role: 'system',
          content: 'You are a task alignment AI. Generate MINIMAL tasks that perfectly fit the existing AI plan schedule. Focus on filling gaps and optimizing existing task flow.'
        },
        {
          role: 'user',
          content: taskPrompt
